...
def on_first_blood(_problem: ProblemAuthLog):
    for bot in Discord.objects.filter(
        is_active=True, subscribe=_problem.problem_instance.problem_list, on_first_blood=True
    ):
...
def on_solved(_problem: ProblemAuthLog):
    for bot in Discord.objects.filter(
        is_active=True, subscribe=_problem.problem_instance.problem_list, on_solved=True
    ):
...
def on_auth_tried(_problem: ProblemAuthLog):
    for bot in Discord.objects.filter(
        is_active=True, subscribe=_problem.problem_instance.problem_list, on_auth_tried=True
    ):
        print(bot.title)
...
def on_problem_updated(_problem: ProblemInstance):
    for bot in Discord.objects.filter(
        is_active=True, subscribe=_problem.problem_list, on_problem_registered=True
    ):
...
def on_question(_question: ProblemQuestion):
    for bot in Discord.objects.filter(
        is_active=True, subscribe=_question.problem_instance.problem_list, on_question=True
    ):
...
def on_answer(_question: ProblemQuestion):
    for bot in Discord.objects.filter(
        is_active=True, subscribe=_question.problem_instance.problem_list, on_answer=True
    ):
